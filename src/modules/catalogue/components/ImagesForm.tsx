...
const ImagesForm: React.FC<ImagesFormProps> = ({ productId, images = [] }) => {
  ...
  async function handleSubmit(values: ImageFormValues, { resetForm }: { resetForm: () => void }) {
    if (!productId) return;
    try {
  ...
  return (
    <div className='card shadow p-4'>
      <div className='grid gap-5'>
        ...
        <Formik initialValues={{ url: '' }} validationSchema={ImageSchema} onSubmit={handleSubmit}>
          {({ isSubmitting }) => (
            <Form className='flex flex-col gap-2 mb-4'>
              <div className='flex flex-col md:flex-row gap-2 items-start'>
                <div className='flex-1'>
                  ...
                  />
                  <ErrorMessage name='url' component='div' className='text-error text-sm mt-1' />
                </div>
                <button
                  type='submit'
