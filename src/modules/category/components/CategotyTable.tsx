...
const CategoryTable: React.FC<Props> = ({ values, regions = [] }) => {
  ...
  const filteredData = useMemo(() => {
    return values.filter((category) => {
      ...
      const matchesRegion = filters.region
        ...
        : true;
      const matchesStatus = filters.isDeleted !== undefined
        ? (filters.isDeleted ? category.isDeleted : !category.isDeleted)
        : true;
  ...
  const data = filteredData.map((category) => ({
    ...
    Region: regions.find((r) => r.code === category.regionId)?.code || 'No asignada',
